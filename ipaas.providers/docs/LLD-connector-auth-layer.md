...
3. Both connectors were called live (via a throwaway test script) and the design was revised to reflect confirmed real behavior — endpoint paths, field names, and pagination — instead of documented guesses. Implemented shortly after as `lib/adapters/connectwise.js` and `lib/adapters/keka.js`.
...
**Out of scope:** webhook receivers; field mapping/transformation (Mapping & Transformation Engine, Day 3); sync scheduling itself (Orchestration Engine, Day 4 — this layer only implements `fetch`/`write`, it doesn't decide when they run); the encryption algorithm's implementation code (defined here at the design level, written only once implementation is approved).
...
| **Known gap** | Primary contact name/email is **not embedded** in the Company record — it's a separate resource at `_info.contacts_href`. Decision needed for Day 3: a second API call per company, or leave canonical `primaryContactName`/`primaryContactEmail` null for this POC (they're already optional in the draft canonical schema, so leaving them null doesn't break anything — recommend this for now, revisit if the demo needs contact data). |
...
**Split between `.env` and Postgres** (unchanged from prior revision):

- `.env` holds only platform bootstrap secrets: the Postgres connection, and `ENCRYPTION_MASTER_KEY`.
- The `credentials` table (Postgres, AES-256-GCM encrypted) holds the actual per-connector config and secrets — one encrypted JSON blob per connector instance.

**Flow:**
1. Raw values (currently in `.env`) get encrypted and inserted into `credentials` via a one-time setup script — **updating** the existing placeholder rows from the Day 1 seed, not inserting new ones (unique constraint on `connector_id`).
2. Once confirmed present in Postgres, raw values are removed from `.env`.
...
Both ConnectWise and Keka are fully custom adapters — confirmed the right call now that both have been called live and neither involves an interactive OAuth consent step Nango would simplify.

## 10. Open Items

- Decide the ConnectWise primary-contact gap (§4): separate API call vs. leave null for the POC. Recommend leaving null for now.
- Confirm ConnectWise's rate limits (not yet tested under real load) before scheduled-sync polling frequency is finalized.
- Run the extended `scripts/test-connectivity.js` locally for ConnectWise Project/Time Entry and Keka Project/Timesheet; paste results back and update §4a/§5a with confirmed endpoints and real field names before writing adapter code.
- Verify Keka's `write()` live for Client while extending it to Project/Timesheet (§5a) — it has only ever been mock-tested, and extending an unverified assumption to two more entities compounds the risk rather than resolving it.
- Confirm the field names ConnectWise uses to reference a Project's parent Company, and Keka uses to reference a Timesheet's parent Project — needed for the canonical schemas' `clientId`/`projectId` fields (see `LLD-mapping-engine.md`).
