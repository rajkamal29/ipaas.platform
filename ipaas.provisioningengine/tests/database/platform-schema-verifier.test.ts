...
} from "../../src/infrastructure/postgres/platform-schema-verifier.js";

const expectedColumns: Record<string, string[]> = {
  tenants: ["id", "name", "created_at"],
  sync_requests: ["id", "tenant_id", "source", "target", "created_at"],
  sync_entities: ["id", "sync_request_id", "entity", "sync_type", "status", "interval_seconds", "created_at", "updated_at"],
  credentials: ["id", "tenant_id", "provider", "encrypted_payload", "iv", "auth_tag", "created_at", "updated_at"],
  sync_state: ["id", "sync_entity_id", "cursor", "last_run_at", "last_run_status", "last_error", "failed", "retry", "created_at", "updated_at"],
  canonical_entities: ["id", "name", "version", "schema", "created_at"],
  mapping_profiles: ["id", "tenant_id", "provider", "entity", "direction", "version", "field_mappings", "is_active", "created_at"],
  global_mapping_profiles: ["id", "provider", "entity", "direction", "version", "field_mappings", "is_active", "created_at"],
};

describe("verifyPlatformSchema", () => {
  it("accepts the current shared platform schema", async () => {
    await assert.doesNotReject(verifyPlatformSchema(createDatabase(), new Logger("error")));
  });
